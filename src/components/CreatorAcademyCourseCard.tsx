  ...
  return (
    <article className="group flex min-h-[27rem] flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-950/45 shadow-[0_20px_60px_rgba(2,6,23,0.18)] backdrop-blur-xl transition hover:-translate-y-1 hover:border-cyan-200/35 focus-within:border-cyan-200/45">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <button type="button" disabled aria-disabled="true" className={`mt-auto min-h-12 cursor-not-allowed rounded-full border px-5 py-3 text-sm font-bold ${isPro ? "border-amber-300/30 bg-amber-300/10 text-amber-100" : "border-cyan-300/25 bg-cyan-300/10 text-cyan-100"}`}>
          {isPro ? "🔒 會員專屬內容" : "查看課程（內容建置中）"}
        </button>
      </div>
    </article>
  );
