...
export function MarketRadarPage({ report }: { report: MarketRadarReport }) {
  return <div className="market-radar-page"><div className="market-radar-page__texture" aria-hidden="true" />
    ...
    <div className="market-radar-container market-radar-content">
      <section className="market-radar-daily-word" aria-labelledby="daily-word"><span className="market-radar-daily-word__bookmark" aria-hidden="true">✦</span><div className="market-radar-daily-word__copy"><p className="market-radar-kicker">TODAY&apos;S KEY TAKE</p><h2 id="daily-word">今日一句</h2><blockquote>高雄房市持續量縮，<br />但不同區域的買氣開始明顯分化。</blockquote></div><svg className="market-radar-daily-word__cityline" viewBox="0 0 620 112" aria-hidden="true"><path d="M0 98h56V75h34V50h24v48h34V27h36v71h28V62h44v36h36V42h46v56h48V20h20v78h38V58h42v40h50V71h42v27h54" fill="none" /><path d="M0 104h620" fill="none" /></svg></section>
      ...
      <section className="market-radar-temperature" aria-labelledby="temperature"><div><p className="market-radar-kicker">MARKET STATUS</p><h2 id="temperature">今日市場溫度 <span>{report.marketTemperature.label}</span></h2><p>{report.marketTemperature.description}</p></div><div className="market-radar-indicators">{report.marketTemperature.indicators.map((indicator) => <article key={indicator.id}><p>{indicator.label} <TrendMark trend={indicator.trend} /></p><strong>{indicator.value}</strong><span>{indicator.detail}</span><i className={`market-radar-meter market-radar-meter--${indicator.trend}`} /></article>)}</div></section>
