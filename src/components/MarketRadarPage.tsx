...
export function MarketRadarPage({ report }: { report: MarketRadarReport }) {
  ...
  return <div className="market-radar-page"><div className="market-radar-page__texture" aria-hidden="true" />
    <section className="market-radar-hero" aria-labelledby="market-radar-title"><div className="market-radar-container market-radar-hero__grid"><div className="market-radar-hero__content"><div className="market-radar-hero__meta"><span>E.X MARKET RADAR</span><b>MOCK DATA · MVP</b></div><p className="market-radar-kicker">Kaohsiung Housing Brief</p><h1 id="market-radar-title">高雄房市快報</h1><p className="market-radar-hero__date">{report.date}｜{report.updatedAt}</p><p className="market-radar-hero__summary">{report.summary}</p><p className="market-radar-hero__notice">本頁為 UI 與 Mock Data 展示，不代表即時市場資訊或投資建議。</p></div><div className="market-radar-hero__download"><MarketRadarDownloadSection report={report} /></div></div></section>
    <div className="market-radar-container market-radar-content">
      <MarketRadarQuickNavigation />
      <section className="market-radar-daily-word" id="market-key-take" aria-labelledby="daily-word"><span className="market-radar-daily-word__bookmark" aria-hidden="true">✦</span><div className="market-radar-daily-word__copy"><p className="market-radar-kicker">TODAY&apos;S KEY TAKE</p><h2 id="daily-word">今日一句</h2><blockquote data-lines={report.dailyKeyTake.lineCount}>{dailyLines.map((line, index) => <span key={`${line}-${index}`}>{line}{index < dailyLines.length - 1 && <br />}</span>)}</blockquote></div><svg className="market-radar-daily-word__cityline" viewBox="0 0 620 112" aria-hidden="true"><path d="M0 98h56V75h34V50h24v48h34V27h36v71h28V62h44v36h36V42h46v56h48V20h20v78h38V58h42v40h50V71h42v27h54" fill="none" /><path d="M0 104h620" fill="none" /></svg></section>
      <section className="market-radar-section" id="market-district-signals" aria-labelledby="district-highlights"><div className="market-radar-section__heading"><div><p className="market-radar-kicker">DISTRICT SIGNALS</p><h2 id="district-highlights">今日 3 大重點</h2></div><span>Free Brief</span></div><div className="market-radar-districts">{report.districtHighlights.map((item) => <article key={item.district}><p>{item.district}</p><h3>{item.headline}</h3><span>{item.note}</span><button type="button" className="market-radar-detail-trigger" onClick={() => setActiveDetail(item.detail)}>查看區域分析 <span aria-hidden="true">→</span></button></article>)}</div></section>
