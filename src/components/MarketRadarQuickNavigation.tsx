...
export function MarketRadarQuickNavigation() {
  ...
  return <nav className="market-radar-quick-navigation" aria-label="Market Radar 快速索引">
    <div className="market-radar-quick-navigation__desktop"><div className="market-radar-quick-navigation__label"><p className="market-radar-kicker">QUICK NAVIGATION</p><span>快速索引</span></div><ol>{navigationItems.map(([id, label], index) => <li key={id}><a href={`#${id}`}><b>{String(index + 1).padStart(2, "0")}</b><span>{label}</span></a></li>)}</ol></div>
