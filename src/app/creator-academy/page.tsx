...
export default function CreatorAcademyPage() {
  return (
    <div className="mx-auto max-w-6xl px-5 py-12 sm:px-8 sm:py-16">
      ...
      <section className="relative mt-8 overflow-hidden rounded-[2rem] border border-violet-300/20 bg-[radial-gradient(circle_at_78%_10%,rgba(124,58,237,0.3),transparent_32%),linear-gradient(140deg,rgba(7,24,54,0.96),rgba(8,9,35,0.92))] shadow-[0_30px_90px_rgba(2,6,23,0.35)]">
        ...
        <div className="relative max-w-3xl">
          ...
          <p className="mt-6 max-w-2xl leading-8 text-slate-300">以 YUNI 作為 AI 創作者導師形象，從 AI 視覺、攝影構圖到影像拍攝，建立可逐步前進的創作學習路徑。</p>
          <a href="#learning-path-title" className="mt-8 inline-flex min-h-12 items-center justify-center rounded-full border border-cyan-200/40 bg-cyan-300/15 px-6 py-3 text-sm font-bold text-cyan-50 transition hover:bg-cyan-300 hover:text-slate-950 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-cyan-200">查看 Learning Path ↓</a>
        </div>
        </div>
        <div className="relative min-h-72 overflow-hidden border-t border-white/10 lg:min-h-full lg:border-l lg:border-t-0"><Image src={publicAssetPath("/images/creator-studio/hero-showcase-v1.png")} alt="YUNI AI 創作者導師形象預留區，呈現創作工作室場景" fill priority sizes="(max-width: 1024px) 100vw, 45vw" unoptimized className="object-cover opacity-60" /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/85 via-transparent to-violet-950/20 lg:bg-gradient-to-r" /><div className="absolute bottom-5 left-5 rounded-2xl border border-white/15 bg-slate-950/70 px-4 py-3 backdrop-blur-xl"><p className="text-[10px] font-bold tracking-[0.16em] text-cyan-200">YUNI CREATOR MENTOR</p><p className="mt-1 text-sm text-slate-300">正式導師形象預留區</p></div></div>
        </div>
      </section>

      <CreatorLearningPath />

      <section aria-labelledby="academy-categories" className="pb-14 sm:pb-20">
