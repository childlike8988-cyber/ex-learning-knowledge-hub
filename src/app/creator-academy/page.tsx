...
import { CreatorLearningNavigator as CreatorAcademyHeroStatus } from "@/components/CreatorLearningNavigator";
...
export default function CreatorAcademyPage() {
  return <div className="mx-auto max-w-6xl px-5 py-12 sm:px-8 sm:py-16">
    ...
    <section className="academy-hero relative mt-8 overflow-hidden rounded-[2rem] border border-violet-300/20 shadow-[0_30px_90px_rgba(2,6,23,0.35)]">
      <div aria-hidden="true" className="academy-hero__circuit" style={{ backgroundImage: `url(${publicAssetPath("/images/creator-academy/hero-circuit-background.png")})` }} />
      <div aria-hidden="true" className="academy-hero__circuit-flow" />
      <div className="relative z-10 grid lg:grid-cols-[1.1fr_0.9fr]">
        <div className="relative px-6 py-12 sm:px-10 sm:py-16"><div className="absolute -right-20 -top-24 h-72 w-72 rounded-full bg-cyan-300/10 blur-3xl" aria-hidden="true" /><div className="relative max-w-3xl"><p className="eyebrow">AI 創作者教育中心 / EDUCATION CENTER</p><h1 className="mt-4 text-4xl font-bold tracking-tight text-white sm:text-6xl">AI 創作者學院</h1><p className="mt-3 text-sm font-bold tracking-[0.13em] text-cyan-100 sm:text-base">Creator Academy</p><p className="mt-6 max-w-2xl leading-8 text-slate-300">從 AI 生成、攝影構圖，到影片製作，打造完整影像創作能力。</p><a href="#learning-path-title" className="mt-8 inline-flex min-h-12 items-center justify-center rounded-full border border-cyan-200/40 bg-cyan-300/15 px-6 py-3 text-sm font-bold text-cyan-50 transition hover:bg-cyan-300 hover:text-slate-950 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-cyan-200">查看學習路徑 ↓</a></div></div>
        <div className="relative min-h-72 overflow-hidden border-t border-white/10 lg:min-h-full lg:border-l lg:border-t-0"><Image src={publicAssetPath("/images/creator-studio/hero-showcase-v1.png")} alt="YUNI AI 創作者導師形象預留區，呈現創作工作室場景" fill priority sizes="(max-width: 1024px) 100vw, 45vw" unoptimized className="object-cover opacity-60" /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/85 via-transparent to-violet-950/20 lg:bg-gradient-to-r" /><AIVideoCenterSwitch /><CreatorAcademyHeroStatus /><div className="absolute bottom-5 left-5 rounded-2xl border border-white/15 bg-slate-950/70 px-4 py-3 backdrop-blur-xl"><p className="text-[10px] font-bold tracking-[0.16em] text-cyan-200">YUNI 導師預留區</p><p className="mt-1 text-sm text-slate-300">YUNI CREATOR MENTOR</p></div></div>
      </div>
    </section>
